        ...
        <li key={i} className="flex items-start gap-3">
          {feature.startsWith('<strong>') ? (
            <div className="mt-1 flex-shrink-0">
                <div className={cn("bg-primary-foreground rounded-full p-0.5", isPopular ? 'bg-primary-foreground' : 'bg-primary')}>
                    <Zap className={cn("w-3 h-3 text-primary", isPopular ? 'text-primary' : 'text-primary-foreground')} strokeWidth={3} />
                </div>
            </div>
          ...
          <span
            ...
            className={cn(isPopular ? 'text-primary-foreground/90' : 'text-foreground')}
          />
        </li>
      ))}
    </ul>
...
const plans = [
    {
        name: 'Basic',
        price: '49',
        description: 'For the casual observer getting started with fundamental analysis.',
        features: [
...
const SampleReportPreview = ({ setActiveTab }: { setActiveTab: (tab: string) => void }) => {
    return (
            ...
            <Card className="shadow-xl overflow-hidden">
              <div className="p-8 border-b bg-secondary/50">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="text-xs font-bold tracking-wider text-muted-foreground uppercase mb-2">Deep Dive • Oct 24, 2024</div>
                    <h2 className="text-3xl font-bold mb-2 font-headline">Palantir (PLTR): The AIP Expansion Thesis</h2>
                    <p className="text-muted-foreground">Analyst: J. Doe, CFA • Price at Publication: $41.50</p>
                  </div>
                ...
                <div className="flex gap-8 text-sm mt-6">
                   {[{l: 'Bear Case', v: '$28.00'},{l: 'Base Case', v: '$45.00'},{l: 'Bull Case', v: '$62.00'}].map(c => (
                     <div key={c.l}>
                       <div className="text-muted-foreground text-xs font-bold uppercase">{c.l}</div>
              ...
              <div className="p-8 relative">
                 <div className="space-y-6">
                   ...
                   <h3 className="font-bold text-lg mb-2 mt-6 text-foreground">The Valuation Disconnect</h3>
                   <p className="mb-4 text-muted-foreground leading-relaxed">
                   ...
                   <div className="my-8 p-6 bg-secondary border rounded-lg">
                      <h4 className="font-bold text-sm mb-4 text-center uppercase tracking-wider text-muted-foreground">DCF Sensitivity Analysis (Preview)</h4>
                      <div className="grid grid-cols-3 gap-4 text-center text-sm">
                         ...
                         <div className="p-2 border-2 border-primary bg-primary text-primary-foreground rounded shadow-lg scale-110">
                            <div className="text-primary-foreground/70 text-xs">WACC 10%</div>
                   ...
                   <h3 className="font-bold text-lg mb-2 text-foreground">Key Risks</h3>
                   <p className="mb-4 text-muted-foreground leading-relaxed">
                 ...
                 <div className="absolute bottom-0 left-0 w-full h-64 bg-gradient-to-t from-background via-background/90 to-transparent flex flex-col items-center justify-end pb-12">
                    ...
                    <h3 className="text-xl font-bold mb-2 text-foreground">Unlock the Full Report</h3>
                    <p className="text-muted-foreground mb-6">Join Pro to access the full thesis and download the Excel model.</p>
                    <Button
                      onClick={() => setActiveTab('plans')}
                      size="lg"
                    >
